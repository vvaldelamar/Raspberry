...
#PUERTA_PEATONAL
find /media/pi/DATOS/DATOS-b/PUERTA_PEATONAL/`date "+%Y-%m-%2d"`/001/jpg/ -name "*.jpg" -type f -printf "%T@\t%Tc %6k KiB %p\n" | sort -n |awk '{print "file "$11}' > image_dahua_01.txt
...
#PUERTA DE ENTRADA
find /media/pi/DATOS/DATOS-b/9F05661PAGB373F/`date "+%Y-%m-%2d"`/001/jpg/ -name "*.jpg" -type f -printf "%T@\t%Tc %6k KiB %p\n" | sort -n |awk '{print "file "$11}' >image_dahua_02.txt

...
#PUERTA_PEATONAL
#/media/pi/DATOS/DATOS-b/PUERTA_PEATONAL/
find /media/pi/DATOS/DATOS-b/PUERTA_PEATONAL/`date "+%Y-%m-%2d" --date "-1 day"`/pic_001 -name "*.jpg" -type f -printf "%T@\t%Tc %6k KiB %p\n" | sort -n |awk '{print "file "$11}' > image_dahua_01.txt
...
find /media/pi/DATOS/DATOS-b/PUERTA_ENTRADA/`date "+%Y-%m-%2d" --date "-1 day"`/001/jpg/ -name "*.jpg" -type f -printf "%T@\t%Tc %6k KiB %p\n" | sort -n |awk '{print "file "$11}' >image_dahua_02.txt
...
#FRENTE_JARDIN
# /media/pi/DATOS/DATOS-b/SENSORES
find /media/pi/DATOS/DATOS-b/SENSORES/`date "+%Y-%m-%2d" --date "-1 day"`/001/jpg/ -name "*.jpg" -type f -printf "%T@\t%Tc %6k KiB %p\n" | sort -n |awk '{print "file "$11}' >image_dahua_03.txt
ffmpeg -y -r 1 -f concat -safe 0 -i image_dahua_03.txt -vcodec h264 -vf "fps=20,scale=-1:720,format=yuv420p,setpts=0.25*PTS" output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.mp4
ffmpeg -y -i output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.mp4 -hls_time 2 -hls_key_info_file /media/pi/DATOS/enc.keyinfo -hls_playlist_type vod -hls_segment_filename "output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`_%d.ts" output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.m3u8
/media/pi/DATOS/spritevideo -i output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.mp4 -o /media/pi/DATOS/video_web -p output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.jpg
ffmpeg -y -i output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.mp4 -r 1 -ss 5 -vframes 1 -s 272x153 output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`_thumb.jpg
rm -rf output_dahua_03_`date "+%Y%m%2d" --date "-1 day"`.mp4
